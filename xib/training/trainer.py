...
from dev_misc.trainlib import Metric, Metrics, freeze, get_grad_norm, get_trainable_params
...
from xib.training.analyzer import AdaptLMAnalyzer, DecipherAnalyzer, ExtractAnalyzer, LMAnalyzer
...
class BaseTrainer(BaseTrainerDev, metaclass=ABCMeta):  # pylint: disable=abstract-method
    ...
    def save_to(self, path: Path):
        to_save = {"model": self.model.state_dict(), "g": g.state_dict(), "optimizer": self.optimizer.state_dict()}
...
class DecipherTrainer(BaseTrainer):
    ...
    def set_optimizer(self):
        super().set_optimizer(
            AdamInverseSqrtWithWarmup, lr=g.learning_rate, betas=(0.9, 0.98), warmup_updates=g.warmup_updates
        )
    ...
    def load(
        self, path: Path, load_lm_model: bool = False, load_optimizer_state: bool = False, load_phi_scorer: bool = False
    ):
...
class ExtractTrainer(BaseTrainer):
    ...
    def train_one_step(self, dl: ContinuousTextDataLoader) -> Metrics:
        ...
        print(
            f"{self.stage}\t{accum_metrics['ll'].value.item()}",
            file=open(os.path.join(os.getcwd(), "ll_logs.tsv"), "a"),
        )
