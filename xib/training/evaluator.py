...
from xib.data_loader import ContinuousIpaBatch, ContinuousTextDataLoader, DataLoaderRegistry
from xib.ipa.process import Segmentation, Span
from xib.model.decipher_model import DecipherModel, DecipherModelReturn, Segmentation, Span
        ...
        with torch.no_grad():
            ...
            for batch in self.data_loader:
                if g.eval_max_num_samples and total_num_samples + batch.batch_size > g.eval_max_num_samples:
                    logging.imp(f"Stopping at {total_num_samples} < {g.eval_max_num_samples} evaluated examples from.")
                    break
...
def get_matching_stats(
    predictions: List[Segmentation], ground_truths: List[Segmentation], match_words: bool = False
) -> Metrics:
...
class DecipherEvaluator(BaseEvaluator):
    ...
    def _get_predictions(self, model_ret: DecipherModelReturn, batch: ContinuousIpaBatch) -> List[Segmentation]:
        ...
        packed_words = self.model.pack(tag_seqs, lengths, batch.feat_matrix, batch.segments, segment_list=segment_list)
...
class ExtractEvaluator(BaseEvaluator):
    ...
    def _get_segmentations(
        ...
        # print(bmnll, self.model.threshold)
