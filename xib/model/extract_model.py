...
class ExtractModel(nn.Module):
    ...
    def __init__(self, lu_size: int):
        ...
        with open(g.vocab_path, "r", encoding="utf8") as fin:
            ...
            self.unit_dense_feat_matrix = {k: v.rename(batch="unit") for k, v in unit_dense_feat_matrix.items()}

        self.adapter = AdaptLayer()
        self.embedding = FeatEmbedding("feat_emb", "chosen_feat_group", "char_emb", dim=g.dim)
    ...
    def forward(self, batch: ExtractBatch) -> ExtractModelReturn:
        ...
        ret = ExtractModelReturn(start, end, best_matched_ll, best_matched_vocab, new_extracted, alignment)

        return ret

    def _extract_one_span(
        self, batch: ExtractBatch, extracted: Extracted, word_repr: FT, unit_repr: FT, char_log_probs: FT
    ) -> Extracted:
    ...
    def _get_matches(
        self, extracted_word_repr: FT, unit_repr: FT, viable_lens: LT, extracted_unit_ids: LT, char_log_probs: FT
    ) -> Matches:
